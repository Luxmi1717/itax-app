...
class _TaxCalculatorState extends State<TaxCalculator> {
  ...
  final TextEditingController residentialStatusController =
  ...
  final TextEditingController taxAfterReliefController =
      TextEditingController();
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        flexibleSpace: Container(
            decoration:
                const BoxDecoration(gradient: AppGradients.mainGradient)),
        leading: IconButton(
            ...
            Expanded(
              child: SingleChildScrollView(
                child: Column(
                  children: [
                    CustomInputField(
                        label: "PAN Number", controller: panController),
                    CustomInputField(
                      label: "Taxpayer Type",
                    ...
                    CustomInputField(
                        label: "Tax Payable",
                        controller: taxPayableController,
                        character: "₹"),
                    CustomInputField(
                        label: "Income Tax after Relief u/s 87A",
                        controller: taxAfterReliefController,
                        character: "₹"),
                    CustomInputField(
                        label: "Surcharge",
                    ...
                    if (show115BAD_BAE) ...[
                      CustomInputField(
                          label: "Opted for Section 115BAD?",
                          controller: section115BADController,
                          isDropdown: true,
                          dropdownItems: ["Yes", "No"]),
                      CustomInputField(
                          label: "Opted for Section 115BAE?",
                          controller: section115BAEController,
                          isDropdown: true,
                          dropdownItems: ["Yes", "No"]),
                    ],
                    if (show115BA_BAA) ...[
                      CheckboxListTile(
                        title: Text(
                            "Company opted and qualify under Section 115BA"),
                        value: section115BA,
                        onChanged: (value) =>
                            setState(() => section115BA = value!),
                      ),
                      CheckboxListTile(
                        title: Text(
                            "Company opted and qualify under Section 115BAA"),
                        value: section115BAA,
                        onChanged: (value) =>
                            setState(() => section115BAA = value!),
                      ),
                    ],
                  ],
                ),
