...
import 'package:itax/presentation/older_screens/ocr/widgets/aadhar-table-widget.dart';
...
class AadhaarOCRScreen extends StatefulWidget {
  const AadhaarOCRScreen({super.key});

  @override
  State<AadhaarOCRScreen> createState() => _AadhaarOCRScreenState();
}

class _AadhaarOCRScreenState extends State<AadhaarOCRScreen> {
  ...
  _handleExtract() {
    ...
    if (token != null && _file != null) {
      print('thing are not null and block provider has started');
      BlocProvider.of<OCRCubit>(context, listen: false)
          .extractAadhaar(file: _file!, token: token!);
    } else {
      print('something is missing $token$_file');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      ...
      if (state is OCRAadhaarSuccessState) {
